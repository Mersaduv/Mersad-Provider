...
interface ParentCategoryProductSliderProps {
  categoryId: string;
  categoryName: string;
  categorySlug: string;
  title?: string;
...
export default function ParentCategoryProductSlider({
  categoryId,
  categoryName,
  categorySlug,
  title,
  ...
  icon,
  limit = 10,
}: ParentCategoryProductSliderProps) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/products/related-categories?categoryId=${categoryId}&limit=${limit}`
      );
  ...
  const checkScrollButtons = useCallback(() => {
    ...
    if (container) {
      ...
      const canRight = scrollLeft < maxScrollLeft - 1;
      ...
      <div className="w-full mb-12">
        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-8 text-center">
          ...
          <p className="text-white/80">
            در حال حاضر محصولی در دسته‌بندی‌های مرتبط با {categoryName} وجود ندارد
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <Link
                ...
                className="bg-white text-indigo-600 px-6 py-3 rounded-lg font-bold hover:bg-gray-100 transition-colors duration-200 flex items-center gap-2"
              >
